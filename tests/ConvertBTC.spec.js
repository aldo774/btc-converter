...
describe('ConvertBTC', () => {
  ...
  it('should use currency USD and 1 as amount default', async () => {
    ...
    expect(consoleStub).to.have.been
      .calledWith(`${chalk.red(1)} BTC to ${chalk.cyan('USD')} = ${chalk.yellow(2490.78)}`);
  });
  ...
  it('should use currency USD and 10 as amount', async () => {
    ...
    expect(consoleStub).to.have.been
      .calledWith(`${chalk.red(10)} BTC to ${chalk.cyan('USD')} = ${chalk.yellow(2490.78)}`);
  });

  it('should use currency BRL and 15 as amount', async () => {
    nock('https://apiv2.bitcoinaverage.com')
      .get('/convert/global')
      .query({ from: 'BTC', to: 'BRL', amount: 15})
      .reply(200, responseMock);

    await convertBTC('BRL', 15);
    expect(consoleStub).to.have.been
      .calledWith(`${chalk.red(15)} BTC to ${chalk.cyan('BRL')} = ${chalk.yellow(2490.78)}`);
  });
